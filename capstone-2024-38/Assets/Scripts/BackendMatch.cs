...
public partial class BackendMatchManager : MonoBehaviour
{
    ...
    // 매칭 서버 접속(서버 접속에 성공한 것이 아니라 단순히 서버와 소켓이 연결된 것)
    ...
    public void RequestMatchMaking()
    {
        ...
        if (isConnectMatchServer)
        {
            Backend.Match.LeaveGameServer(); //인게임 서버에 접속되어 있을 경우를 대비해 인게임 서버 리브 호출
        }
    ...
    private void ProcessMatchMakingResponse(MatchMakingResponseEventArgs args)
    {
        ...
        switch (args.ErrInfo)
        {
            case ErrorCode.Success:
                ...
                //LobbyUI.GetInstace().MatchDoneCallBack();
                ProcessMatchSuccess(args);
                break;
            case ErrorCode.Match_InProgress:
                // 매칭 신청을 성공했을 때 or 매칭 중일 때 매칭 신청을 시도했을 때
            
                // 매칭 신청을 성공했을 때
                if (args.Reason == string.Empty)
                {
                    debugLog = SUCCESS_REGIST_MATCHMAKE;
                    //Lobby.UI.GetInstance().MatchRequestCallBack(true);
                }
                break;
            case ErrorCode.Match_MatchMakingCanceled:
                ...
                //Lobby.UI.GetInstance().MatchRequestCallBack(false);
                break;
        ...
        if (!debugLog.Equals(string.Empty))
        {
            ...
            if (isError == true)
            {
                //Lobby.UI.GetInstance().SetErrorObject(debugLog);
            }
        }
    }
    
    // 매칭에 성공했을 때, 인게임 서버로 접속해야 한다
